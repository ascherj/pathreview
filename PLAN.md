...
- **Interaction with `street_address`:** `PII_PATTERNS` is a dict processed
  in insertion order (email, phone_us, phone_intl, ssn, street_address).
  While testing I found `street_address`'s regex has an unrelated
  pre-existing bug: it can greedily match ordinary prose (e.g.
